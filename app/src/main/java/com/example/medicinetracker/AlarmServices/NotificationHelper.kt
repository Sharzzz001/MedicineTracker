...
class NotificationHelper(base: Context?) : ContextWrapper(base) {
    ...
    fun getChannelNotification(): NotificationCompat.Builder? {
        ...
        var timeAM=DateFormat.format(delegate, Calendar.getInstance().time)
        //Get Medicine name
        FirebaseFirestore.getInstance().collection("alarms").whereEqualTo("puid", name).whereArrayContains(
            "alarmtext",
            timeAM).get()
            .addOnSuccessListener { documents ->
                for (document in documents) {
                    //Log.d(TAG, "${document.id} => ${document.data}")
                    MedName = document!!["MedName"] as String
                }
            }
        //End
        ...
        return NotificationCompat.Builder(applicationContext, channelID)
            ...
            .setContentText("It's $formatted, time to take your medicine")
